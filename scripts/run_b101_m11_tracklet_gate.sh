...
OUT_PRED_ROOT="${OUT_PRED_ROOT:-$MOSE_WORKSPACE/homework/pred_sam2_m11_tracklet}"
SUBMIT_ROOT="${SUBMIT_ROOT:-$MOSE_WORKSPACE/homework/submission_433_m11_tracklet}"
ZIP_PATH="${ZIP_PATH:-$MOSE_WORKSPACE/homework/submission_mosev2_m11_tracklet.zip}"
AUDIT_JSON="${AUDIT_JSON:-$MOSE_WORKSPACE/homework/logs/m11_tracklet_gate_latest.json}"
DRY_RUN="${DRY_RUN:-0}"
...
if [[ -n "$GATE_EXTRA_ARGS" ]]; then
  # Simple space-separated flags, e.g.
  # GATE_EXTRA_ARGS="--far-dist-frac 0.16 --area-grow-ratio 4.0"
