...
- Qualitative evidence: visual comparison completed; see `docs/m2_visual_analysis.md` and `artifacts/m2_memory_gate/visual_analysis/`. Main conclusion: M2 improves memory hygiene but often over-suppresses tiny targets or follows stable wrong same-class distractors.
...
- Decision: code-review blockers fixed and submission zip validated/downloaded locally, but visual evidence does **not** support preferring M2 over original SAM2 as final submission. Keep as diagnostic/ablation.
- Next: design M3 around identity-aware reactivation/re-detection after occlusion, rather than threshold-only memory gating.
