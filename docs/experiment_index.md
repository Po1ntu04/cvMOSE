...
| M7 Qwen-VL verifier | Use Qwen-VL / Aliyun Bailian as cached target profiler, candidate verifier, and delayed-promotion judge for M5R-C. | `src/cvmose/qwen_vl_client.py`, `src/cvmose/mllm_panels.py`, `tools/mllm_target_profile.py`, `tools/mllm_candidate_judge.py`, `tools/mllm_tracklet_judge.py`, `docs/m7_qwen_vl_verifier.md` | Real `qwen3.5-plus` smoke works; 16 target profiles succeeded; q0 same-class candidate judge correctly vetoes/uncertains bad guinea-pig anchors. | Current proven effect is safety, not recovery: q0 b101 smoke after the fix rejects all pseudo-anchors and copies baseline. No final zip yet. |
