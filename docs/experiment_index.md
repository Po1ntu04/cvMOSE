...
| M7 DINO part-tracklet | Upgrade DINO mean pooling to local patch/part matching and aggregate candidate margins over short source/motion-consistent tracklets. | `src/cvmose/dino_descriptors.py`, `tools/infer_mosev2_sam2_reanchor.py`, `docs/m7_part_tracklet_report.md`, `docs/assets/m7_part_tracklet/smoke/` | Better verification: suppresses weak `msinig6m` anchor and keeps `2smf7uq9` with stronger evidence. | Does not solve candidate recall for `r13u5z4y`/`lcgc29va`; use as focused probe, not final mainline. |
